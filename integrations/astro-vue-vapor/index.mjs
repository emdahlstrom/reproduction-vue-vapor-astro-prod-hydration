...
// surface (createApp/createSSRApp/h/…) PLUS Vapor + vaporInteropPlugin in one
// file, so the SFC and the client renderer share a single runtime copy (one
// `currentInstance`). The modular `vue.runtime.esm-bundler.js` entry can't be
// used: it re-exports runtime-dom and runtime-vapor with `export *`, so names
// both export (createApp/createSSRApp) tree-shake to `undefined`.
//
// `astro build` ships the PRODUCTION build, which has the two `__DEV__`-gated
// vapor bugs this repo reproduces (see README); patches/vue@3.6.0-beta.17.patch
// fixes them. `astro dev` keeps the DEV build, which hydrates correctly.
